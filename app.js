(function(){
  ...
  if(grid){
    grid.innerHTML=content.courses.sort((a,b)=>a.order-b.order).map((c,i)=>`<a class="course-card accent-${esc(c.accent)} ${c.featured?'featured':''}" href="course.html?c=${encodeURIComponent(c.slug)}">
      <div class="course-visual"><img class="card-photo" src="assets/sample-photo.jpg" alt=""><span class="course-index">${String(i+1).padStart(2,'0')}</span><span class="course-status">${esc(c.status)}</span><div class="lens-ring"></div><div class="focus-mark">+</div></div>
  ...
  if(tgrid){ tgrid.innerHTML=content.testimonials.map((t,i)=>`<article class="testimonial-card ${t.featured?'testimonial-featured':''}"><div class="testimonial-media"><img src="assets/sample-photo.jpg" alt=""><span class="play">▶</span><span class="video-label">VIDEO TESTIMONIO</span></div><div class="testimonial-copy"><small>${esc(t.course)}</small><blockquote>“${esc(t.quote)}”</blockquote><p>${esc(t.name)}</p></div></article>`).join(''); }
  const ngrid=q('#news-grid');
  if(ngrid){ ngrid.innerHTML=content.news.map((n,i)=>`<a class="news-card ${i===0?'news-featured':''}" href="${esc(n.link)}"><div class="news-thumb"><img src="assets/sample-photo.jpg" alt=""></div><div class="news-date">${esc(n.date)}</div><small>${esc(n.tag)}</small><h3>${esc(n.title)}</h3><p>${esc(n.text)}</p><b>Leer más ↗</b></a>`).join(''); }
  const sgrid=q('#services-grid');
  if(sgrid){ sgrid.innerHTML=content.services.map((s,i)=>`<article class="service-card"><div class="service-photo"><img src="assets/sample-photo.jpg" alt=""></div><span class="service-no">0${i+1}</span><div><small>${esc(s.eyebrow)}</small><h3>${esc(s.title)}</h3><p>${esc(s.text)}</p><a href="https://wa.me/54${esc(site.whatsapp)}" target="_blank" rel="noopener">${esc(s.cta)} ↗</a></div></article>`).join(''); }
  const cgrid=q('#community-grid');
  if(cgrid){ cgrid.innerHTML=content.community.map((c,i)=>`<a class="community-card community-${i+1}" href="${esc(c.link)}"><div class="community-photo"><img src="assets/sample-photo.jpg" alt=""></div><small>${esc(c.label)}</small><h3>${esc(c.title)}</h3><p>${esc(c.text)}</p><b>Explorar ↗</b></a>`).join(''); }
